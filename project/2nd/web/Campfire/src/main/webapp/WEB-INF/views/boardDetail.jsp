...
<script type="text/javascript">
	$(function() {
		...
		/* 글쓴이와 로그인한 회원의 번호가 일치할때만 수정, 삭제 버튼 보이게 하기 */
		/* $('#btnDelete').hide()
		$('#btnUpdate').hide() */
		
		var nick = $('#nickname').val()
		var vo_nick = $('#nick').val()
		console.log('세션 닉네임 : '+nick)
		console.log('글쓴이 닉네임 :'+vo_nick)
		...
		/* 신고 클릭시 */
		$('#i-rp').click(function(){
			var result = confirm('해당 글을 신고하시겠습니까?');
			//alert(result)
			if (result) {
		...
		$.getJSON('/listComments?bno=' + bno, function(arr) {
			...
			$.each(arr, function(idx, data) {
				...
				var span = $('<span class="span"></span>') // 댓글 한 묶음 감싸기
				...
				/* var blank = $('<span class="span"></span>')
				for(var i=0; i<=level; i++){
					var img = $('<img class="re" alt="Image placeholder"></img>').attr({src: 'img/re.png', width:'20px', height:'100px'})
					$(blank).append(img)
				}  */
				...
				var blank = $('<span class="span"></span>')
				for(var i=0; i<=level; i++){
					var img = $('<i class="fas fa-reply fa-rotate-180"></i>')/* .attr({src: 'img/r.png', width:'10px', height:'15px'}) */
					$(blank).append(img)
				}

				var text = $('<p></p>')
				var contents = $('<p class="span"></p>').html(data.ctext)
				$(text).append(blank, contents)
				var reply = $('<p></p>')
				...
				$(span).append(vcard, comment_body)
				//$(wrap).append(blank ,span)
				$(comment).append(span)
				...
				$(a).click(function() {
					...
					var f_nickname = $("<input type='text' name='nickname' size='50' readonly='readonly'>").val(nick)
				...
				<form id='F' action="#" class="p-5 bg-light">
					<div class="comment-form-div">
						<label for="nickname">작성자 *</label> <input type="text"
							name="nickname" id="nickname" value="${nickname}" readonly="readonly"
							class="form-control">
					</div>
					<div class="comment-form-div">
					...
					<div class="comment-form-div">
						...
						<input type='hidden' id='nick' name='nick' value='${vo.nickname }'>
